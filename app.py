...
.player{width:95%;max-width:1100px;position:relative;background:black;display:flex;align-items:center;justify-content:center;}
video{width:100%;max-height:100vh;object-fit:contain;}
.controls{position:absolute;bottom:0;width:100%;padding:12px;background:linear-gradient(to top,rgba(0,0,0,.9),transparent);transition:.3s;display:flex;flex-direction:column;}
button{background:none;border:none;color:white;font-size:18px;margin:4px;cursor:pointer;}
.progress{height:6px;background:#374151;margin-bottom:8px;cursor:pointer;position:relative;}
.played{position:absolute;height:100%;background:#ef4444;width:0%;}
.buffer{position:absolute;height:100%;background:#6b7280;width:0%;}
.center{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);font-size:60px;color:white;cursor:pointer;}
.settingsMenu{position:absolute;bottom:70px;right:15px;background:rgba(0,0,0,.95);border-radius:12px;display:none;flex-direction:column;min-width:220px;color:white;}
...
<video id="video" src="/stream?url={{url}}" crossorigin="anonymous" preload="auto"></video>
...
// PiP
pip.onclick=async()=>{if(document.pictureInPictureElement)document.exitPictureInPicture();else video.requestPictureInPicture()}
